...
def INFORMATION_MAIN_MENU(WEBSITE_TO_SCAN):
    BANNER()
    print("    Target => {DMN.netloc}".format(DMN=urlparse(WEBSITE_TO_SCAN)))
    while True:
        print("\n    [1]: Scan For Sensitive HTTP-Header(s)")
        print("\n    [98]: Go back")
        print("    [99]: Exit WP-Broker\n")
        BRUTEFORCE_MENU_CHOICE = input("    [WP-Broker@Informations]~[Root] #> ")
        if BRUTEFORCE_MENU_CHOICE == "1":
            gather_info.GetInfo(WEBSITE_TO_SCAN)
